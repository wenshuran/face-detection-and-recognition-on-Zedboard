...
void vdma_start_triple_buffering(vdma_handle *handle_s2mm, vdma_handle *handle_mm2s) {
    ...
    // Wait for reset to finish
    while((vdma_get(handle_s2mm, OFFSET_VDMA_S2MM_CONTROL_REGISTER) & VDMA_CONTROL_REGISTER_RESET)==4){
        printf("s2mm control: %x\n", vdma_get(handle_s2mm, OFFSET_VDMA_S2MM_CONTROL_REGISTER));
    }
    printf("s2mm reset finish\n");
    while((vdma_get(handle_mm2s, OFFSET_VDMA_MM2S_CONTROL_REGISTER) & VDMA_CONTROL_REGISTER_RESET)==4){
        printf("mm2s control: %x\n", vdma_get(handle_mm2s, OFFSET_VDMA_MM2S_CONTROL_REGISTER));
        vdma_set(handle_mm2s, OFFSET_VDMA_MM2S_CONTROL_REGISTER, VDMA_CONTROL_REGISTER_RESET);
	sleep(1);
    }
;
    ...
    vdma_set(handle_mm2s, OFFSET_VDMA_MM2S_STATUS_REGISTER, 0);
 
    // Do not mask interrupts
    vdma_set(handle_s2mm, OFFSET_VDMA_S2MM_IRQ_MASK, 0xf);
 
    int interrupt_frame_count = 1;
 
    // Start both S2MM and MM2S in triple buffering mode
    vdma_set(handle_s2mm, OFFSET_VDMA_S2MM_CONTROL_REGISTER,
        (interrupt_frame_count << 16) |
        VDMA_CONTROL_REGISTER_START |
        //VDMA_CONTROL_REGISTER_GENLOCK_ENABLE |
        //VDMA_CONTROL_REGISTER_INTERNAL_GENLOCK |
        VDMA_CONTROL_REGISTER_CIRCULAR_PARK);
    vdma_set(handle_mm2s, OFFSET_VDMA_MM2S_CONTROL_REGISTER,
        (interrupt_frame_count << 16) |
        VDMA_CONTROL_REGISTER_START |
        VDMA_CONTROL_REGISTER_GENLOCK_ENABLE |
    ...
    while((vdma_get(handle_s2mm, 0x30)&1)==0 || (vdma_get(handle_s2mm, 0x34)&1)==1
        || (vdma_get(handle_mm2s, 0x30)&1)==0 || (vdma_get(handle_mm2s, 0x34)&1)==1) {
        printf("Waiting for VDMA to start running...\n");
        sleep(1);
    }
    printf("Status set up over\n");

    // Extra register index, use first 16 frame pointer registers
    vdma_set(handle_s2mm, OFFSET_VDMA_S2MM_REG_INDEX, 0);
 
    // Set park write number
    //vdma_set(handle_s2mm, OFFSET_PARK_PTR_REG, 0x00000100);

    // Write physical addresses to control register
    vdma_set(handle_s2mm, OFFSET_VDMA_S2MM_FRAMEBUFFER1, handle_s2mm->fb1PhysicalAddress);
    vdma_set(handle_mm2s, OFFSET_VDMA_MM2S_FRAMEBUFFER1, handle_mm2s->fb1PhysicalAddress);
 
    // Write Park pointer register
    vdma_set(handle_s2mm, OFFSET_PARK_PTR_REG, 0);
    vdma_set(handle_mm2s, OFFSET_PARK_PTR_REG, 0);
 
    // Frame delay and stride (bytes)
    vdma_set(handle_s2mm, OFFSET_VDMA_S2MM_FRMDLY_STRIDE, handle_s2mm->width*handle_s2mm->pixelLength);
    vdma_set(handle_mm2s, OFFSET_VDMA_MM2S_FRMDLY_STRIDE, handle_mm2s->width*handle_mm2s->pixelLength);
 
    // Write horizontal size (bytes)
    vdma_set(handle_s2mm, OFFSET_VDMA_S2MM_HSIZE, handle_s2mm->width*handle_s2mm->pixelLength);
    vdma_set(handle_mm2s, OFFSET_VDMA_MM2S_HSIZE, handle_mm2s->width*handle_mm2s->pixelLength);
 
    // Write vertical size (lines), this actually starts the transfer
    vdma_set(handle_s2mm, OFFSET_VDMA_S2MM_VSIZE, handle_s2mm->height);
    vdma_set(handle_mm2s, OFFSET_VDMA_MM2S_VSIZE, handle_mm2s->height);
