...
int cvtImage(int Handler, unsigned int srcAddr, unsigned int dstAddr){
    long* srcdata = (long*) mmap(NULL, 640*480*2, PROT_READ | PROT_WRITE, MAP_SHARED, Handler, srcAddr);
    long* dstdata = (long*) mmap(NULL, 640*480*2, PROT_READ | PROT_WRITE, MAP_SHARED, Handler, dstAddr);

    CvMat mCvmat = cvMat(640, 480, CV_8UC2, srcdata);
    ...
    memcpy((void *)dstdata, (void *)dstImage.ptr<double>(0), 640*480*2);
...
int main() {
    ...
    while(1){
    	if(vdma_get(&handle_s2mm, OFFSET_VDMA_S2MM_STATUS_REGISTER) & VDMA_STATUS_REGISTER_HALTED == 1){
          cvtImage(Handler, 0x10000000, 0x11000000);
    	  vdma_start_s2mm(&handle_s2mm);
        }
    } 
